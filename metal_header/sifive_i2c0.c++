...
void sifive_i2c0::create_defines() {
  ...
  dtb.match(std::regex(compat_string), [&](node n) {
    ...
    n.maybe_tuple("baud", tuple_t<uint32_t>(),
                  [&]() {
                    emit_def("__METAL_DT_I2C0_BAUDRATE" + std::to_string(i),
                             std::to_string(0));
                  },
                  [&](uint32_t baud) {
                    emit_def("__METAL_DT_I2C0_BAUDRATE" + std::to_string(i),
    ...
    if (n.field_exists("master")) {
      emit_def("__METAL_DT_I2C0_MODE" + std::to_string(i), "METAL_I2C_MASTER");
    }

    if (n.field_exists("slave")) {
      emit_def("__METAL_DT_I2C0_MODE" + std::to_string(i), "METAL_I2C_SLAVE");
    }
...
void sifive_i2c0::create_handles() {
  ...
  if (num_i2cs) {
    int i = 0;
    dtb.match(std::regex(compat_string), [&](node n) {
      emit_def_handle("__METAL_DT_SIFIVE_I2C0_HANDLE" + std::to_string(i), n,
                      ".i2c");
      i++;
    });
  }
